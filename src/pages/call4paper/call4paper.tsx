...
export default function Call4Paper() {
  ...
  const handleInsert = async (e: any) => {
    ...
    if (!formData.title) {
      alert('Por favor, indique el titulo')
      return
    }
    if (!formData.descripcion) {
      alert('Por favor, rellena la descripción')
      return
    }
    if (!formData.firstName) {
      alert('Por favor, ponga su nombre')
      return
    }
    if (!formData.lastName) {
      alert('Por favor, ponga su apellido')
      return
    }
    if (!formData.email) {
    ...
    if (!formData.bio) {
      alert('Por favor, rellene la bio')
      return
    }
    if (!formData.nickname) {
      alert('Por favor, rellena el nickname')
      return
    }
    if (!formData.residence) {
      alert('Por favor, indique su residencia')
      return
    }
  ...
  return (
    <>
      <Navbar />
      <div>
        <p className={`${styles.mainText}`}>¿QUIERES SER PONENTE?</p>
      ...
      <form className={`${styles.form}`} onSubmit={handleInsert}>
        ...
        <textarea
          name="bio"
          placeholder="Una pequeña bio (90 caracteres)"
          className={`${styles.inputFormArea}`}
          onChange={handleChange}
          value={formData.bio}
        ></textarea>
        ...
        <textarea
          name="notes"
          placeholder="¿Algo que debamos tener en cuenta? (comida, intolerancias, etc.)"
          className={`${styles.inputFormArea}`}
          onChange={handleChange}
        ...
        <textarea
          name="experience"
          placeholder="Experiencia Previa en Actividades del Estilo"
          className={`${styles.inputFormArea}`}
          onChange={handleChange}
        ...
        <input
          type="text"
          name="company"
          placeholder="Empresa de trabajo (y posibilidad de patrocinio)"
          className={`${styles.inputForm}`}
          onChange={handleChange}
